...
var Ogg = function (_Event) {
    ...
    function Ogg(channel) {
        ...
        _this.queue = [];
        _this.flushLimit = 50; /* the larger flush limit, the lesser clicking noise */
        _this.init();
        return _this;
    }
    ...
    createClass(Ogg, [{
        ...
        value: function getOGG() {
            var oggData = this.oggHeader,
                ...
                segmentData = void 0,
                headerType = void 0;
            var processing = this.flushLimit
            while (processing--) {
                packet = this.queue.shift();
                headerType = 0//this.queue.length == 0 ? 4 : 0; // for last packet, header type should be end of stream
        ...
        value: function process() {
            ...
            this.audioCtx.decodeAudioData(ogg.buffer, function (audioBuffer) {
                _this2.dispatch('data', audioBuffer);
            });
        }
    }, {
...
var OpusToPCM = function (_Event) {
    ...
    function OpusToPCM(options) {
        ...
        if (nativeSupport) {
            ...
            _this.decoder = new OpusWorker(options.channels, options.libopusPath);
        } else {
            _this.decoder = null;
        }
